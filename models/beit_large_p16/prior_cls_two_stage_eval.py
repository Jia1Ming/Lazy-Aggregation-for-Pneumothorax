...
def load_model_and_processor():
    global _PROCESSOR, NUM_PREFIX
    _PROCESSOR = AutoImageProcessor.from_pretrained(MODEL_ID)
    trunk = AutoModel.from_pretrained(MODEL_ID, add_pooling_layer=False).to(DEVICE).eval()
    NUM_PREFIX = 1
    print("Loaded:", MODEL_ID, "| trunk:", type(trunk).__name__, "| prefix tokens:", NUM_PREFIX)
...
def _run_full_tokens(trunk, pixel_values):
    return trunk(pixel_values).last_hidden_state
...
def split_cls_patch(hidden):
    return hidden[0], hidden[NUM_PREFIX:NUM_PREFIX + NUM_PATCHES]
...
def _lung_masked_pv(image, keep_idx):
    pv = preprocess(image).clone()
    keep = set(int(i) for i in keep_idx)
    for idx in range(NUM_PATCHES):
        if idx not in keep:
            r, c = idx // NUM_PATCHES_1D, idx % NUM_PATCHES_1D
            pv[0, :, r * PATCH_SIZE:(r + 1) * PATCH_SIZE, c * PATCH_SIZE:(c + 1) * PATCH_SIZE] = 0.0
    return pv

def extract_lung_only_features(trunk, image, keep_idx):
    hidden = get_last_hidden(trunk, _lung_masked_pv(image, keep_idx))
    cls, patches = split_cls_patch(hidden)
    keep = torch.as_tensor([int(i) for i in keep_idx], dtype=torch.long)
    return cls, patches.index_select(0, keep)
...
assert get_last_hidden(trunk, _dummy).shape[0] == NUM_PREFIX + NUM_PATCHES
