...
def load_model_and_processor():
    global _NORM_MEAN, _NORM_STD, _FULL_REL, _LOCAL_UP, _USE_POS
    trunk = timm.create_model(MODEL_ID, pretrained=True)
    gs_old = int(round(trunk.patch_embed.num_patches ** 0.5))
    if gs_old != NUM_PATCHES_1D:
        pe = trunk.pos_embed.data.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
        pe = F.interpolate(pe, size=(NUM_PATCHES_1D, NUM_PATCHES_1D), mode="bicubic", align_corners=False)
        trunk.pos_embed = torch.nn.Parameter(pe.permute(0, 2, 3, 1).reshape(1, NUM_PATCHES, -1))
        trunk.patch_embed.strict_img_size = False
    trunk = trunk.to(DEVICE).eval()

    _USE_POS = bool(getattr(trunk, "use_pos_embed", True))
    _LOCAL_UP = int(trunk.local_up_to_layer)
    dc = timm.data.resolve_model_data_config(trunk)
    _NORM_MEAN = np.asarray(dc["mean"], dtype=np.float32)
    _NORM_STD  = np.asarray(dc["std"],  dtype=np.float32)
    _FULL_REL = trunk.blocks[0].attn.get_rel_indices(NUM_PATCHES).to(DEVICE)

    print("Loaded:", MODEL_ID, "| trunk:", type(trunk).__name__,
          "| dim:", trunk.embed_dim, "| blocks:", len(trunk.blocks),
          "| local_up:", _LOCAL_UP, "| INPUT:", f"{IMAGE_SIZE}x{IMAGE_SIZE}", "->", NUM_PATCHES, "patches")
    return trunk, None

def _set_gpsa_rel(rel):
    for blk in trunk.blocks:
        if hasattr(blk.attn, "rel_indices"):
            blk.attn.rel_indices = rel
...
def _run_full_tokens(trunk, pixel_values):
    _set_gpsa_rel(_FULL_REL)
    return trunk.forward_features(pixel_values)
...
def forward_features_keep_from_pv(trunk, pixel_values, keep_idx):
    pixel_values = pixel_values.to(DEVICE, dtype=next(trunk.parameters()).dtype)
    keep = torch.as_tensor([int(i) for i in keep_idx], dtype=torch.long, device=DEVICE)
    with torch.no_grad():
        x = trunk.patch_embed(pixel_values)
        if _USE_POS:
            x = x + trunk.pos_embed
        x = trunk.pos_drop(x)
        x = x.index_select(1, keep)
        rel_sub = _FULL_REL.index_select(1, keep).index_select(2, keep).contiguous()
        _set_gpsa_rel(rel_sub)
        cls = trunk.cls_token.expand(x.shape[0], -1, -1)
        for u, blk in enumerate(trunk.blocks):
            if u == _LOCAL_UP:
                x = torch.cat((cls, x), dim=1)
            x = blk(x)
        x = trunk.norm(x)
    return x.float().cpu()[0]
...
def save_panels_for_image(row, band_px=18, lung_thr=LUNG_THR, with_masking=True):
    stem = row["stem"]
...
def save_panels_batch(n_img=None, seed=0, band_px=18, lung_thr=LUNG_THR, masking_n=30):
    sel_rows, mode = select_rows(n_img, seed)
    make_panel_dirs()
    print(f"Rendering panels ({mode}); "
          f"masking curve on first {masking_n} ...")
    failed = []
    for i, row in enumerate(sel_rows):
        try:
            save_panels_for_image(row, band_px=band_px, lung_thr=lung_thr,
                                  with_masking=(i < masking_n))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--panel_masking_n", type=int, default=30,
                    help="how many panel images also get the (expensive) masking-curve panel "
                         "(rendered on the first N in selection order; the other 5 kinds cover all)")
    ...
    if args.panels != 0:
        save_panels_batch(n_img=args.panels, seed=args.seed, band_px=args.band_px,
                          lung_thr=args.lung_thr, masking_n=args.panel_masking_n)
    else:
