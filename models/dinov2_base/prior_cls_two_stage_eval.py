...
def load_model_and_processor():
    ...
    trunk = AutoModel.from_pretrained(MODEL_ID).to(DEVICE).eval()
    NUM_PREFIX = 1
...
def _run_full_tokens(trunk, pixel_values):
    return trunk(pixel_values, interpolate_pos_encoding=True).last_hidden_state
...
def forward_features_keep_from_pv(trunk, pixel_values, keep_idx):
    pixel_values = pixel_values.to(DEVICE, dtype=next(trunk.parameters()).dtype)
    keep_tokens = list(range(NUM_PREFIX)) + [NUM_PREFIX + int(i) for i in keep_idx]
    with torch.no_grad():
        hidden = trunk.embeddings(pixel_values)
        hidden = hidden[:, keep_tokens, :]
        hidden = trunk.encoder(hidden)[0]
        hidden = trunk.layernorm(hidden)
    return hidden.float().cpu()[0]

def extract_lung_only_features(trunk, image, keep_idx):
    hidden = forward_features_keep_from_pv(trunk, preprocess(image), keep_idx)
    return hidden[0], hidden[NUM_PREFIX:]
