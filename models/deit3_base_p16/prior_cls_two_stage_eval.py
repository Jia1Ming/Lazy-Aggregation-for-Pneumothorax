...
def _run_full_tokens(trunk, pixel_values):
    ...
    x = getattr(trunk, "patch_drop", torch.nn.Identity())(x)
    x = getattr(trunk, "norm_pre", torch.nn.Identity())(x)
    x = trunk.blocks(x)
    x = trunk.norm(x)
    return x
...
def forward_features_keep_from_pv(trunk, pixel_values, keep_idx):
    pixel_values = pixel_values.to(DEVICE, dtype=next(trunk.parameters()).dtype)
    keep_patch = [int(i) for i in keep_idx]
    with torch.no_grad():
        x = trunk.patch_embed(pixel_values)
        x = trunk._pos_embed(x)
        x = getattr(trunk, "patch_drop", torch.nn.Identity())(x)
        x = getattr(trunk, "norm_pre", torch.nn.Identity())(x)
        keep = [0] + [p + 1 for p in keep_patch]
        x = x[:, keep, :]
        x = trunk.blocks(x)
        x = trunk.norm(x)
